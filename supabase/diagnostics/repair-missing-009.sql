-- ============================================================
-- Repair: migration 009 was never applied
--
-- Symptom this fixes: the dashboard reports
--   dashboard_snapshot  42883  function normalize_item_type(text) does not exist
--   lending_metrics / jewellery_stock / inventory_report  PGRST202  not found
--
-- Cause. Migration 009 creates twelve report functions. Ten of them exist
-- nowhere else, so if 009 never ran they are simply absent. The two
-- exceptions are dashboard_stats and chart_data, which later migrations
-- re-create -- which is exactly why those two kept working and hid the
-- problem for months.
--
-- It surfaced only when migration 032 was applied because 032 rewrote
-- dashboard_snapshot to group the safe inventory by normalize_item_type().
-- The 030 version never called it, so the missing function was invisible
-- until then. Nothing 031 or 032 did was wrong; they were the first code to
-- depend on a gap that already existed.
--
-- ORDER MATTERS, and this is the trap this file exists to prevent. Running
-- 009 on its own would overwrite the newer dashboard_stats and chart_data
-- with 009's older versions -- verified, not assumed: doing that drops
-- closed_amount, interest_count and prev_closed_amount from dashboard_stats
-- (the Removals card silently reads zero) and reverts chart_data from
-- SECURITY DEFINER to INVOKER. So 022 and 032 are replayed afterwards to put
-- them back.
--
-- Safe to run on live data: every statement is CREATE OR REPLACE FUNCTION or
-- GRANT. No table is created, altered or written to.
--
...
-- ============================================================
-- Tell PostgREST about the new functions immediately.
--
-- Without this the app keeps returning PGRST202 for them until the cache
-- happens to refresh, which looks identical to the migration not having run.
-- ============================================================
